...
_RUN_CMD 'tee /home/'${username}'/assets/scenario/conf/hashicups-api-1/agent-acl-tokens.hcl > /dev/null << EOF
acl {
  tokens {
...
_RUN_CMD 'tee /home/'${username}'/assets/scenario/conf/hashicups-api-1/svc-hashicups-api.hcl > /dev/null << EOF
## -----------------------------
...
_RUN_CMD -o plaintext 'ls /home/'${username}'/assets/scenario/conf/hashicups-api-1'

md_log '
Copy the configuration on the remote node.'

_RUN_CMD 'scp -r -i '${SSH_CERT}' /home/'${username}'/assets/scenario/conf/hashicups-api-1/* '${username}'@hashicups-api-1:/etc/consul.d/'
...
## Give time to Consul process to settle
## [ux-diff] [cloud provider] UX differs across different Cloud providers 
if [ "${SCENARIO_CLOUD_PROVIDER}" == "docker" ]; then
